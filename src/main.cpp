...
#if defined(NDEBUG)
# error "Coorgcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd50e77ed99a621ac16151d02faabce7479d9b4c40ce9897a9ea34bbe5073e7d4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Coorgcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Coorgcoin Signed Message:\n";
...
{
    // Coorgcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Coorgcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Coorgcoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Coorgcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Coorgcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Coorgcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf7f11c5efe06724a98712fb02774017915c9b5cbb3e2116e8c9c3c4c180a12d4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Coorgcoin is the Future";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1543873781;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085724096;

        if (fTestNet)
        {
            block.nTime    = 1543873756;
            block.nNonce   = 386420714;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xf1b5dddadf7fc1988490d03b7adb21cd2ffcc771a5bbaeaf20dfe8b6769aabf7"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Coorgcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CoorgcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CoorgcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CoorgcoinMiner(CWallet *pwallet)
{
    printf("CoorgcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("coorgcoin-miner");
    ...
    try { loop {
        ...
        printf("Running CoorgcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CoorgcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CoorgcoinMiner, pwallet));
}
